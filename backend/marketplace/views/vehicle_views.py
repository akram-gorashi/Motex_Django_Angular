...
class VehicleViewSet(viewsets.ModelViewSet):
    ...
    """
    queryset = Vehicle.objects.filter(is_active=True).select_related('seller', 'model__brand').prefetch_related('images')
    serializer_class = VehicleSerializer
    permission_classes = [permissions.IsAuthenticatedOrReadOnly]  # Public can view, only authenticated users can create/edit

    # ✅ Enable Filtering & Sorting
    filter_backends = [DjangoFilterBackend, filters.OrderingFilter]
    filterset_fields = ['model__brand__name', 'model__name', 'year', 'fuel_type', 'transmission', 'price']
